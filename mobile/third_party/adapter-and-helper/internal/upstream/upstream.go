...
type Upstream struct {
	...
	// netDialContext, if set, is used for the outbound WS dial instead of
	// the default net.Dialer. Added for the Android/gomobile port so the
	// bridge connection can be protect()-ed. See SetNetDialContext.
...
// SetNetDialContext installs a custom dial function for the upstream
// WebSocket connection (e.g. one that calls Android's VpnService.protect()
// on the underlying socket, so this connection isn't captured by our own
// TUN). Must be called before Run(). A nil value restores the default
// dialer behaviour.
//
// Added for the Android/gomobile port — not present in upstream helper.exe.
func (u *Upstream) SetNetDialContext(dial func(ctx context.Context, network, addr string) (net.Conn, error)) {
	u.mu.Lock()
	u.netDialContext = dial
	u.mu.Unlock()
}
...
func (u *Upstream) dial(ctx context.Context) (*websocket.Conn, error) {
	...
	u.mu.Lock()
	netDial := u.netDialContext
	u.mu.Unlock()
	dialer := websocket.Dialer{
		EnableCompression: false,
		HandshakeTimeout:  10 * time.Second,
		NetDialContext:    netDial, // nil = gorilla/websocket's default dialer
	}
