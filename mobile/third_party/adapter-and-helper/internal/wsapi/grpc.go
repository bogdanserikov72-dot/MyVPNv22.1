...
type grpcClient struct {
	...
	// dialContext, if set, replaces gRPC's default dialer (e.g. to call
	// Android's VpnService.protect() on the socket). Set via
	// NewClientWithDialer. Added for the Android/gomobile port.
...
func (g *grpcClient) ensure() (ws.ConnectionServiceClient, error) {
	...
	if g.dialContext != nil {
		opts = append(opts, grpc.WithContextDialer(g.dialContext))
	}
